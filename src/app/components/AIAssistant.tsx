  ...
  const formatContent = (text: string) => {
    return text.split("\n").map((line, i) => {
      ...
      while ((match = boldPattern.exec(line)) !== null) {
        ...
        parts.push(<strong key={match.index} className="text-foreground font-medium">{match[1]}</strong>);
        ...
        <div className="fixed bottom-20 right-6 w-80 sm:w-96 h-[520px] bg-card border border-border rounded-xl shadow-2xl flex flex-col z-50 overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3">
            {messages.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                <div
                  className={`max-w-[85%] rounded-xl px-3 py-2.5 text-xs leading-relaxed space-y-1 ${
                    msg.role === "user"
          ...
          <div className="border-t border-border px-3 py-3 flex gap-2">
            ...
            <button
              ...
              className="w-8 h-8 rounded-lg bg-primary text-primary-foreground flex items-center justify-center disabled:opacity-40 hover:bg-primary/90 transition-colors"
            >
              <Send size={13} />
            </button>
          </div>
